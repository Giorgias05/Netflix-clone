...
function Film(props) {
    ...
    useEffect(() => {
        const Fetch = async () => {
            const r = await fetch(`http://www.omdbapi.com/?apikey=68aafa04&s=${props.film}`);
                ...
                {data ? (
                    data.slice(0, 6).map((HarryPotter) => (
                        <div key={HarryPotter.imdbID} className="col mb-2 px-1">
                            <img className="img-fluid" src={HarryPotter.Poster}
                             alt={HarryPotter.Title} style={{ height: '240px', width: '310px' }} />
                        </div>
                    ))
                ) : (
